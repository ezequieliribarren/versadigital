import React from 'react'

const Gastronomico = () => {
  return (
    <div className='subtitleRubro'><h3>Nuestras recomendaciones para tu negocio</h3>
    <div className='rubro'>
<div className='items'>
    <div className='titleRubro'>
        <span className='nroItem'>1.</span><h4>imagen de marca</h4>  
    </div>
<div className='imgRubro'>
    <img src="/images/gastronomico/maskGastronomico.webp" alt="Imagen de Marca" />
</div>
<div className='descripRubro'>
   <p>Destacá, genera confianza y lealtad con una imagen de marca profesional para tu restaurante-bar</p> 
</div>
</div>
<div className='linea'></div>
<div className='items'>
    <div className='titleRubro'>
        <span className='nroItem'>2.</span><h4>sitio de presentación</h4>  
    </div>
<div className='imgRubro'>
    <img src="/images/gastronomico/sitioPresentacion.webp" alt="Sitio Web" />
</div>
<div className='descripRubro2'>
    <div>
          <p>Creamos el sitio de presentación ideal para tu restaurante-bar, 100% personalizado y adaptado a tus necesidades</p> 
    </div>

    <div className='vector1'>
           <img src="/images/vectores/flechaSubir.png" alt="Aumento de precensia" />
    </div>
 <p>Aumenta tu <br/>  presencia en línea</p>
<div className='vector2'>
     <img src="/images/vectores/clientes.png" alt="Atraer Clientes" />
</div>
   <p>Atrae nuevos <br/> clientes</p>
</div>
</div>
<div className='linea2'></div>
<div className='items'>
    <div className='titleRubro'>
        <span className='nroItem'>3.</span><h4>sistema de reservas</h4>  
    </div>
<div className='imgRubro'>
    <img src="/images/gastronomico/turnos.webp" alt="Sistema de reservas" />
</div>
<div className='descripRubro2'>
    <div>
          <p>Agrega un sistema de reservas en tu sitio web, optimizando tiempos y brindando una mejor experiencia</p> 
    </div>

    <div className='vector1'>
           <img src="/images/vectores/calendario.png" alt="Aumento de precensia" />
    </div>
 <p>Aumenta tu <br/>  presencia en línea</p>
<div className='vector2'>
     <img src="/images/vectores/reloj.png" alt="Atraer Clientes" />
</div>
   <p>Atrae nuevos <br/> clientes</p>
</div>
</div>



    </div>
    <div className='contactoRubro'>
    <button><a target="_blank" href="https://wa.me/+541130589482?text=Hola,%20estoy%20en%20la%20página%20de%20VERSA DIGITAL%20y%20quiero%20 pedir%20más%20información">contactanos</a></button>
    </div>
    </div>
  )
}

export default Gastronomico